...
select2 = df[df['location'].isin(countries2)]

plt.figure(figsize=(15,7))
sns.lineplot(data=select2, x='date', y='new_cases_smoothed', hue='location')
plt.title('Згладжено кількість нових випадків')
plt.xlabel('Роки')
plt.ylabel('Кількість випадків')
...
plt.figure(figsize=(15,7))
sns.lineplot(data=select2, x='date', y='new_deaths_smoothed', hue='location')
plt.title('Згладжено кількість нових смертей')
plt.xlabel('Роки')
...
fig = px.line(select2,x='date',y='people_vaccinated',color='location',title='Динаміка вакцинації', labels={'date':'Дати', 'people_vaccinated':'Кількість вакцинованих людей', 'location':'Колір графіків країн'})
